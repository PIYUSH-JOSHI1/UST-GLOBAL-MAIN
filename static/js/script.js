...
function initializeReports() {
    ...
    mainContent.innerHTML = reportContent;
    
    createPredictionChart();
    createDensityChart();
    createCongestionChart();
    
    document.getElementById('junctionSelect').addEventListener('change', updateCharts);
    ...
    window.predictionChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                label: 'Predicted Green Signal Time (s)',
                data: [30, 25, 35, 40],
...
function updateCharts() {
    const junctionId = document.getElementById('junctionSelect').value;
    
    // Simulate fetching new data
    const newData = {
        prediction: {
            vehicleCount: [Math.floor(Math.random() * 100), Math.floor(Math.random() * 100), Math.floor(Math.random() * 100), Math.floor(Math.random() * 100)],
        },
        density: [Math.random() * 100, Math.random() * 100, Math.random() * 100, Math.random() * 100, Math.random() * 100, Math.random() * 100, Math.random() * 100, Math.random() * 100],
        congestion: [Math.random() * 100, Math.random() * 100, Math.random() * 100, Math.random() * 100, Math.random() * 100]
    };
    
    // Calculate predicted green signal timing
    const maxSignalTime = 60; // Assuming maximum signal time is 60 seconds
    const maxTraffic = Math.max(...newData.prediction.vehicleCount);
    const densityFactor = maxSignalTime / maxTraffic;
    
    const predictedGreenSignalTime = newData.prediction.vehicleCount.map(count => {
        // Using the formula: Green Signal Time = Density factor * Predicted traffic
        // Where Predicted traffic = (Vehicle Count[-1] + Vehicle Count[-2] + Vehicle Count[-3]) / 3
        // For simplicity, we'll use the current vehicle count as an approximation
        return Math.round(densityFactor * count);
    });
    
    // Update prediction chart
    window.predictionChart.data.datasets[0].data = newData.prediction.vehicleCount;
    window.predictionChart.data.datasets[1].data = predictedGreenSignalTime;
    window.predictionChart.update();
    
    // Update density chart
    window.densityChart.data.datasets[0].data = newData.density;
    window.densityChart.update();
    
    // Update congestion chart
    window.congestionChart.data.datasets[0].data = newData.congestion;
